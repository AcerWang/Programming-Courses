...
Let's look back at previous `mybatis.cfg.xml`.
...
The configuration file contains thses tags:
...
- configuration
  ...
  In our previous configuration file, only `typeAliases`, `environments`, `mappers`.


###2.3 properties

We do the operation based on previous demo project.
...
> - Then, read `resource` in `properties`, load `property` in resource url specified file and override loaded same name `property`.
> - Last, load `property` passed as param and cover loaded same name `property`.

### 2.4 settings

`settings` is important in configuration file. It would change MyBatis behavior, such as enable L2 cache, lazy load, etc.
...
`environments` is for datasource. You can config several `environments` to adapt to different databases.
...
`dataSource` use standard JDBC interface to config JDBC connection object. There are three types dataSource: type = "[UNPOOLED|POOLED|JNDI]".
...
`JNDI` supports outside dataSource connection pool. It can be used in container such as EJB or application server. It contains properties:
