...
Let's see previous `UserMapper.xml` content:
...
The mapper file contains a `mapper` tag which also contains some sub tags. This file stores SQL statements. Mapper file reduces code compared to implementing such operation in Java code, it's much better and configurable.
...
`resultMap` is the most powerful and complicated part in mapper file. For simple SQL statement, no need to specify resultMap explicitly. For complicated SQL statement, it needs.
...
Here, the `resultType` is `User`, an alias for `com.labex.mybatis.model.User`. In this case, MyBatis will automatically create a resultMap which will map columns of a row to JavaBean properties.
...
The select mapping statement, we can see the `reslutMap` is a `userMap`.
...
First, you need to define a constructor taking parametors for a JavaBean.
...
Sometimes, a single select statement returns multiple results. `discriminator` is for this. It works like switch in Java.
...
Only when set is "male", it maps to `set` property.
...
> returnType and returnMap can not exist all at once.
...
Here, `useGeneratedKeys` will create primary key, `keyProperty` indicated the primary key column.
...
By default, using`#{}` to set value is safe and fast. If you want to insert a immutable string into a SQL statement, you can use `${}`, MyBatis will not change and transfer the string.

> This is not safe, may suffer SQL injection attact.
