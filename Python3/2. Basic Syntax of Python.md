...
Input the code after prompt >>> .First you can input print("hello labex").

It is not difficult to exit interactive environment, either by shortcut key Ctrl-D or imputting code exit().
...
In particular, the first two characters #! are called Shebang, aiming to tell shell to use Python 3 interpreter to execute codes.
...
We need to put ./ ahead when executing a script to represent it is under the current directory.
...
1. Vim is not easy to get started. Please turn to sublime or gedit if needed.
2. Space character is significant for Python. In the example, all code are required to put by the left side, otherwise, there will be mistakes.

This is a complete operation video for program writing:

`@

https://labfile.oss-cn-hangzhou.aliyuncs.com/courses/923/1-2.flv
...
Pay attention to str4 and str5 do not have \，but they can still use quotation, and I believe you have already discovered the reason.
...
Do not mix Space and TAB. We strongly suggest only using Space. To maintain a good style of coding, four Space as the indent is recommended.
...
In this simple procedure, we will use the operation that will be discussed in later lab. Function open() to open a file, and function read() to read file contents.
...
If the error happens to f.write() when writing data to file, close can not be executed.Using finally can make sure the content in code block will be carried out and closed files no matter block try will detect exceptions or not.
...
This is all for this lab. Things are very basic, since it is almost impossible to introduce Python comprehensively in a single lab, and these are the most common points：
...
Please type all the sample code in the file on your own, and try not to copy and paste. Because you can be familiar with Python code by typing and practicing. If you have a question, drop us a message in the discussion forum. The basic syntax of Python 3 is not difficult, while the toughest thing is to persist on programming and always have a desire to learn.
